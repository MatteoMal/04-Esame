...
class AccediController extends Controller
{
    ...
    public static function testLogin($hashUtente, $hashPassword) {
        if (Auth::esisteUtenteValidoPerLogin($hashUtente)) {
            //esiste
            $auth = Auth::where('user', $hashUtente)->first();
            //$auth->sfida = $sfida;
    ...
    public static function controlloPassword($utente, $hashClient){
        ...
        if (Auth::esisteUtenteValidoPerLogin($utente)) {
           ...
           // echo("PASSO 1: ".$inizioSfida." - ".$durataSfida);
    ...
    public function cifra($utente)
    {
        return hash("sha512", trim($utente));
    }
}
